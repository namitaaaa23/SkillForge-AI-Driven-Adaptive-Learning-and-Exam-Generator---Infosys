            ...
            if (isSignUp) {
                ...
                if (result.success) {
                    ...
                    if (result.data.role !== 'ADMIN') {
                        const actualRole = result.data.role === 'STUDENT' ? 'Learner' : 'Guardian';
                        const loginUrl = result.data.role === 'STUDENT' ? '/learner-login' : '/guardian-login';
                        setErrors({ password: `You are a ${actualRole}. Please login at the ${actualRole} login page.` });
                        return;
                    }
                    ...
                    if (errorMsg.toLowerCase().includes('not found') || errorMsg.toLowerCase().includes('no user') || errorMsg.toLowerCase().includes('does not exist')) {
                        setErrors({
                            email: `No account exists with this email. Please sign up to create a new account.`
                        });
                    } else {
